...
for filename in os.listdir(input_folder):
    ...
    try:
        ...
        # If PDF has only 1 page -> move original
        if num_pages == 1:
            ...
            print(f"→ Moved single-page PDF to: {dest_path}")
 
        # If multi-page -> move original + split
        elif num_pages > 1:
            # Move original
            ...
            # Split pages → ALL into ONE folder
            for i, page in enumerate(reader.pages):
                writer = PdfWriter()
                writer.add_page(page)
 
                output_filename = f"{base_name}_page_{i+1:03d}.pdf"
                output_path = os.path.join(split_pages_folder, output_filename)
 
                # Avoid overwrite
                if os.path.exists(output_path):
                    name, ext = os.path.splitext(output_filename)
                    j = 1
                    while os.path.exists(os.path.join(split_pages_folder, f"{name}_{j}{ext}")):
                        j += 1
                    output_path = os.path.join(split_pages_folder, f"{name}_{j}{ext}")
 
                with open(output_path, "wb") as f:
                    writer.write(f)
 
            print(f"Split complete for {filename}. ({num_pages} pages dumped into split_pages)")
 
        else:
            print(f"Skipping {filename}: 0 pages detected")
